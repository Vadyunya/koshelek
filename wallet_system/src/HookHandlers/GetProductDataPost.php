...
class GetProductDataPost{
    ...
    function getProductDataPost(&$product_data, $auth, $preview, $lang_code) {
                            ...
                            if($promotion_condition['condition'] == 'products'){
                                foreach($promotion_condition['value'] as $promotion_product){
                                    if($product_id == $promotion_product['product_id']){
                                        $product_data['product_promotion'] = $promotion_bonuses['1']['discount_value'];
                                        $product_data['product_promotion_type'] = $promotion_bonuses['1']['discount_bonus'];
                                        $product_data['product_promotion_quantity'] = $promotion_product['amount'];
