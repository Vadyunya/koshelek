...
class IndexPost extends BaseController{
    /**
     * IndexPost constructor.
     *
     * @param string $mode
     */
    ...
    public function index() {
        ...
        && fn_is_expired_storage_data('send_feedback', SECONDS_IN_DAY * 30)) 
        {
            ...
            $this->response = array(CONTROLLER_STATUS_REDIRECT, $redirect_url);
        }
        ...
        && in_array($_REQUEST['time_period'], $time_periods)) 
        {
        ...
        || fn_check_view_permissions('taxes.manage', 'GET')) 
        {
            ...
            //changes//
            $orders_stat['orders_total'] = $this->loadModel->displayOrderTotals($orders_stat['orders']);
            $time_difference = $timestamp_to - $timestamp_from;
            $params = array(
                'period' => 'C',
            ...
            $orders_stat['prev_orders_total'] = $this->loadModel->displayOrderTotals($orders_stat['prev_orders']);
        ...
        if (fn_check_view_permissions('cart.cart_list', 'GET')) {
            ...
            $orders_stat['abandoned_cart_total'] = count($this->loadModel->getUserSessionProducts($timestamp_from,$timestamp_to,$company_condition));

            $orders_stat['prev_abandoned_cart_total'] = count($this->loadModel->getUserSessionProductsDiffrence($timestamp_from,$timestamp_to,$company_condition,$time_difference));
        ...
        if (fn_check_view_permissions('orders.manage', 'GET')) {
            ...
            $order_by_statuses = $this->loadModel->getOrdersData($timestamp_from,$timestamp_to,$company_condition);
        }
        /* /Order by statuses */
        ...
        $graphs = fn_dashboard_get_graphs_data($timestamp_from, $timestamp_to, $is_day,$company_id,$storefront_id);
        /* /Statistics */
